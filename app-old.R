...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    selectInput( # Utilizar esse menu para colocar Crop
      inputId = "airline",
      label = "Airline:",
      choices = airline_list,
      selected = "DL", # DL representa Delta Airlines
      selectize = FALSE
    ),
    sidebarMenu(
      selectInput( # Aqui para selecionar o Ano
        inputId = "month",
        label = "Month:",
        choices = month_list,
        selected = 99, # representa todos os meses na lista de mes
        size = 13,
        selectize = FALSE
      ),
  ...
  dashboardBody(
    tabsetPanel(
      ...
      tabPanel(
        ...
        value = "page1",
        fluidRow( # Aqui estão as 3 caixas que ficam no topo do reporte
          valueBoxOutput("total_flights"), # Talvez colocar o total de híbridos
          valueBoxOutput("per_day"), # Total de locais
          valueBoxOutput("percent_delayed") # Total de plots
        ),
        fluidRow(), # Uma linha em Branco
        fluidRow(
          column(
            width = 6,
            d3Output("group_totals") # Primeiro grafico
          ),
          column(
            width = 6,
            d3Output("top_airports") # Segundo grafico
          )
        )
      )
    )
...
server <- function(input, output, session) {
  ...
  base_flights <- reactive({
    res <- flights %>% # dataframe flights existe por conta de um pacote
